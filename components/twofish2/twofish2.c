...
#include	"twofish2/debug.h"				/* debug display macros */
...
int twofish2_crypt(int mode, int keySize, char* in, char* keyMaterial, int dir_encrypt) /* keySize must be 128, 192, or 256 */
{																			 /* return 0 iff test passes */
	...
	memset(plainText, 0, TEXT_SIZE);
	memset(cipherText, 0, TEXT_SIZE);
	memcpy(plainText, in, byteCnt);
	// strcpy(cipherText, cf);
	// memset(decryptOut, 0, TEXT_SIZE);

	if (makeKey(&ki, dir_encrypt/*DIR_ENCRYPT*/, keySize, keyMaterial) != TRUE) 
	...
	if (mode != MODE_ECB) /* set up random iv (if needed)*/
	{
		for (i = 0; i < sizeof(iv); i++)
			iv[i] = (BYTE)(i ^ (i * 1093 + 7))   ;//esp_random();
